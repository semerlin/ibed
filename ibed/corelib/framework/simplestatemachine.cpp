...
static SimpleStateMachine::Transition string2Struct(
        const SimpleStateMachine::TransitionDescription &transDesc,
...
{
    StringTransition stringTransition = parseOneLine(oneLine);

    Transition transition = string2Struct(stringTransition, m_callee->metaObject());

    addTransition(stringTransition.source, transition);
}
...
const SimpleStateMachine::TransitionList& 
SimpleStateMachine::assertFind(const QString &str){
...
bool SimpleStateMachine::postEvent(const QString& event){
    ...
    QString findEvent(event);

    while(true){

        TransitionList::const_iterator transitionPos=
            std::find_if(findBegin, findEnd, EventFinder(findEvent));
...
SimpleStateMachine::TransitionDescription SimpleStateMachine::parseOneLine(const QString &oneLine)
{
